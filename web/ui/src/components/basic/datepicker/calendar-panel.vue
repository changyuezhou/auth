  ...
  <div class="calendar">
    ...
    <div class="calendar-content">
      ...
      <div class="calendar-year" v-show="currentPanel === 'years'">
        <a v-for="(year,index) in years" :key="index" @click="selectYear(year)" :class="{'current': currentYear === year}">{{year}}</a>
      </div>
      <div class="calendar-month" v-show="currentPanel === 'months'">
        <a v-for="(month, index) in months" :key="index" @click="selectMonth(index)" :class="{'current': currentMonth === index}">{{month}}</a>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data () {
    const translation = this.$parent.zh
    return {
